// routes/incomplete.js
import express from "express";
import { pool } from "../db.js";

const router = express.Router();

// GET /api/incomplete - List all jobs with incomplete/missing items
router.get("/", async (_req, res) => {
  const sql = `
    ...
    SELECT
      ...
      /* all "needs" notes logged for this install job */
      COALESCE((
      ...
      /* purchasing for the production job (via bid_id -> bids.job_id -> jobs.id) */
      COALESCE((
        SELECT jsonb_agg(p.*)
        FROM public.bids b
  ...
  try {
    ...
    console.error('[INCOMPLETE] query error', e);
    res.status(500).json({ error: 'query_failed', detail: e.message });
  }
});

// GET /api/incomplete/:jobId - Get details for a specific incomplete job
router.get("/:jobId", async (req, res) => {
  ...
  try {
    ...
    console.error('[INCOMPLETE] detail error', e);
    res.status(500).json({ error: 'query_failed', detail: e.message });
  }
});

// POST /api/incomplete/:jobId/resolve - Mark incomplete items as resolved
router.post("/:jobId/resolve", async (req, res) => {
  ...
  try {
    ...
    // Mark all pending purchase items as resolved/canceled
    await client.query(
    ...
    // Update service tasks to complete or cancel
    await client.query(
    ...
    console.error('[INCOMPLETE] resolve error', e);
