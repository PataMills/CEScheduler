...
import db from "../db.js"; // update if your db helper has a different path/name
...
router.get("/api/sales/jobs", async (req, res) => {
  ...
  try {
    ...
    console.error('[GET /api/sales/jobs]', e.message);
    res.status(500).json([]);
  }
});
...
router.post("/api/services", async (req, res) => {
  ...
  // Optional: store service request meta (skip if you want)
  await db.query(
...
router.get("/api/bids/:id/package", async (req, res) => {
  ...
  const { rows:docs } = await db.query(
    `SELECT id, kind, name, url, uploaded_at FROM bid_documents WHERE bid_id=$1 ORDER BY uploaded_at DESC`, [id]
  );
...
router.post("/api/bids/:id/ready-for-schedule", async (req, res) => {
  const { id } = req.params;
  await db.query(`UPDATE bids SET status='ready_for_schedule' WHERE id=$1`, [id]);
  res.json({ ok: true });
});
...
router.get('/api/bids/:id/customer-info', async (req, res) => {
  ...
  try {
    // Try by bid id first
    const { rows: byBid } = await db.query(
    ...
    // Fallback: treat :id as install_jobs.id
    const { rows: byJob } = await db.query(
    ...
    console.error('[GET /api/bids/:id/customer-info]', e.message);
